...
<script>
  ...
  function createNotification(msg) {
    Notify({ position: 'bottom right',type: 'danger', duration: 4000, title: msg  });
...
<!-- ---------------------------- Notifications End --------------------- -->
    ...
    <?php
              ...
              require('db.php');
//-------------------------------------------------------------- Check membership expiry & Expire ------------
              
              $memberend = "";
              if ($_SESSION["membership"] != "FREE"){

                $uname11 =  $_SESSION["uname"];
                $sql112="SELECT * FROM seller_info WHERE username='$uname11' AND memberend < now() " ;
                $result112 = $conn->query($sql112);
                
                if($row112 = $result112->fetch_assoc())
                {

                  $memberend = $row112['memberend'];

                  $sql1121="UPDATE seller_info set membership='FREE' WHERE username='$uname11'" ;
                  $result1121 = $conn->query($sql1121);
    ...
    <ul class="navbar-nav ml-auto">
      ...
      <li class="nav-item dropdown">
        <a class="nav-link"  href="http://localhost/trueshipp/index.php">          
          Home 
        </a>
      ...
      <li class="nav-item dropdown">
        <a class="nav-link"  href="http://localhost/trueshipp/notifications.php">
          <i class="far fa-bell"></i>
          <span class="badge badge-warning navbar-badge"><?php echo $cnt119; ?></span>
        </a>
 ...
 <?php
    ...
    ?>  
     ...
     <li class="nav-item d-sm-inline-block">
        <a href="<?php echo $path1; ?>TDash.php" class="nav-link">Dashboard<i class="fas fa-angle-right right "></i></a>
        
      </li>
      <li class="nav-item d-sm-inline-block">
        <a href="<?php echo $path1; ?>pages/transporter/vehicles.php" class="nav-link">Vehicles<i class="fas fa-angle-right right"></i></a>
      </li>
      <li class="nav-item d-sm-inline-block">
    ...
    <li class="nav-item d-sm-inline-block">
        <a href="<?php echo $path1; ?>pages/transporter/membership.php" class="nav-link">Membership<i class="fas fa-angle-right right"></i></a>
      </li>
    <li class="nav-item d-sm-inline-block">
...
<?php
  ...
  $sql1="SELECT * FROM products Where status1='Delivered' AND tusername='".$_SESSION["uname"]."'" ;					
	...
	$sql1="SELECT * FROM products Where userEmail='".$_SESSION["uname"]."' AND typepost='SALE' " ;
	$result1 = $conn->query($sql1);							
	while($row1 = $result1->fetch_assoc())
	{	
		$sl_cnt++;  	
	...
	$sql1="SELECT * FROM products Where userEmail='".$_SESSION["uname"]."' AND typepost='BOOK' " ;
	$result1 = $conn->query($sql1);							
	while($row1 = $result1->fetch_assoc())
	{	
		$rn_cnt++;  	
  ...
  $sql1="SELECT * FROM products Where userEmail='".$_SESSION["uname"]."'  " ;
      ...
      while($row1 = $result1->fetch_assoc())
      {
        $pid1 =$row1['product_id'];     
          ...
          $sql3="SELECT * FROM product_seller Where product_id=$pid1" ;								
          $result3 = $conn->query($sql3);

          //$bd_cnt=0;
          while($row3 = $result3->fetch_assoc())
          {
            $accept_reject = $row3['accept_reject'];    
        ...
        <div class="row mb-2">
          <div class="col-sm-12">
            ...
            <h1 class="m-0 text-dark">Dashboard - 
              <img src='./assets/imagesu/<?php echo $_SESSION["uname"]; ?>.jpg?t=<?php echo filemtime('./assets/imagesu/'.$_SESSION["uname"].'.jpg'); ?>' style="height: 40px; width: 40px; border-radius:30px; pointer-events: none; border-radius:30px;  object-fit: cover;" /> 
              - <?php echo $memberend; ?>
            <?php 
            ...
            echo "<h6><br> Membership: ".$_SESSION["membership"]. "</h6>";
            ?>
            </h1>
          </div><!-- /.col 
          <div class="col-sm-6">
        ...
        <div class="row">
          <div class="col-lg-3 col-6">
            ...
            <div class="small-box" style="background-color: #ff8000;">
              ...
              <a href="http://localhost/trueshipp/pages/transporter/vehicles.php" class="small-box-footer">More info <i class="fas fa-arrow-circle-right"></i></a>
            </div>
          </div>
          <!-- ./col -->
          ...
          <div class="col-lg-3 col-6">
            ...
            <div class="small-box " style="background-color: 	#00bfff;">
              <div class="inner">
                <h3><?php echo $rn_cnt; ?><sup style="font-size: 20px"></sup></h3>
